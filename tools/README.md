...
These python scripts are used to manipulate XYZ potential energy surfaces (PESs). Make sure the files are executable and that you have python3 installed (tested on python 3.13.9).
...
This allows tiling of a periodic PES. Flipping every other tile supported with the options `--flip-x`, `--flip-y` and `--flip-xy`.
Example use of 3x2 tiling:
```bash
./xyz_tile_pes.py -x3 -y2 <example.pes >tiled.pes
```

If we denote the whole PES in `example.pes` with `#`, the file `tiled.pes` would be equivalent to
```
###
###
```
Please refer to the module docstring for more information.

## fort_to_xyz / traj_tsv_to_xyz_no_config

Helper scripts for converting TSV files to XYZ trajectories.
